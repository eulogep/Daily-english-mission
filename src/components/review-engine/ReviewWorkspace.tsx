  ...
  return (
    <Card className="mx-auto max-w-3xl border-emerald-900/15 bg-white shadow-lg">
      ...
      <CardContent className="space-y-5 p-6">
        ...
        {feedback && !feedback.correct && <div className="space-y-3"><div className="flex flex-wrap gap-2"><Button onClick={() => retry(item.id)}>Réessayer</Button><Button variant="outline" onClick={() => finish(false, null)}>Revoir bientôt</Button>{hintCount === 0 && <Button variant="ghost" onClick={() => showHint(item.id)}><HelpCircle />Voir un indice</Button>}</div>{hintCount > 0 && <p className="rounded-lg bg-slate-50 px-4 py-3 text-sm text-slate-700"><strong>Indice :</strong> {item.hint}</p>}<p className="text-xs text-slate-500">Retries dans cette révision : {retryCount}</p></div>}
